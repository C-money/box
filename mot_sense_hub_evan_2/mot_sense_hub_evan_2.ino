...
int sound_tap_queue[4] = {0, 0, 0, 0};

#define N_SCENES 4
// scene timeouts in seconds
int scene_timeouts[N_SCENES] = {0, 0, 0, 0};
...
void setup() {
  ...
  pinMode(scene_change_pin, INPUT_PULLUP);
...
void loop() {
  ...
  static int last_scene_change_pin = 0;
  int current_scene_change_pin;
  ...
  current_scene_change_pin = digitalRead(scene_change_pin);
  if (current_scene_change_pin != last_scene_change_pin) {
    scene_change_count++;
    if (scene_change_count > 10) {
      last_scene_change_pin = current_scene_change_pin;
      if (current_scene_change_pin == HIGH) {
        current_scene++;
        if (current_scene == 16) {
          current_scene = 0;
        }
  ...
  for (i = 0; i < OUTPUT_PACKET_SIZE; i++) {
    ...
    //Serial.printf("%03d\n", output_packet[i]); // readable debug
    
    //Serial.printf("%c", output_packet[i]);
    //HWSERIAL.printf("%c", output_packet[i]);  // DON'T enable on Evan's test bed
  }
  //Serial.printf("\n");
  ...
  for (i = 0; i < N_MOTION_SENSORS_ACTIVE; i++) {
    if (output_packet[i + OUTPUT_PACKET_HEADER_SIZE] != 0) {
      if (output_packet[i + OUTPUT_PACKET_HEADER_SIZE] / 16 + 1 > sound_tap_queue[i]) {
